...
SELECT
  ...
  C.nombre,
  C.compositor,
  COUNT(CP.id_playlist)
FROM
  canciones AS C
  JOIN canciones_de_playlists AS CP ON C.id = CP.id_cancion
GROUP BY
  C.id
HAVING
  COUNT(CP.id_playlist) >= 5;
...
SELECT
  SUM(F.precio_unitario * F.cantidad) AS total_general
FROM
  facturas AS F;
...
FROM
  clientes AS C
  JOIN facturas AS F ON C.id = F.id_cliente
GROUP BY
  C.id;
...
SELECT
  F.*,
  SUM(CF.precio_unitario * CF.cantidad)
FROM
  facturas AS F
  JOIN items_de_facturas AS CF ON F.id = CF.id_factura
GROUP BY
  F.id;

...
SELECT * FROM movies WHERE awards > (SELECT AVG(awards) FROM movies);
...
SELECT * FROM movies WHERE awards > (SELECT AVG(awards) FROM movies WHERE awards > 0 AND awards IS NOT NULL);


-- 3. Generar una consulta que evalúe todos los actores que no han actuado en una
-- película ni tampoco en una serie.
SELECT A.*, COUNT(AE.id), COUNT(AM.id) FROM actors AS A LEFT JOIN actor_episode AS AE ON A.id=AE.actor_id LEFT JOIN actor_movie AS AM ON A.id=AM.actor_id GROUP BY A.* HAVING COUNT(AE.id)=0 AND COUNT(AM.id)=0;
